...
//        auxiliaryTables.eraseRegexp();
    ...
    private void columnNew() throws SQLException {
        ...
        //Добавление столбцов с регулярными выражениями для поиска совпадений по столбцам
        statement.execute("ALTER TABLE AUXILIARY.KLADR ADD COLUMN IF NOT EXISTS REGEXP_DISTRICT VARCHAR(MAX) DEFAULT ('');");
        statement.execute("ALTER TABLE AUXILIARY.KLADR ADD COLUMN IF NOT EXISTS REGEXP_TYPE_CITY VARCHAR(MAX) DEFAULT ('');");
        statement.execute("ALTER TABLE AUXILIARY.KLADR ADD COLUMN IF NOT EXISTS REGEXP_CITY VARCHAR(MAX) DEFAULT ('');");
        statement.execute("ALTER TABLE AUXILIARY.KLADR ADD COLUMN IF NOT EXISTS REGEXP_TYPE_LOCALITY VARCHAR(MAX) DEFAULT ('');");
        statement.execute("ALTER TABLE AUXILIARY.KLADR ADD COLUMN IF NOT EXISTS REGEXP_LOCALITY VARCHAR(MAX) DEFAULT ('');");
        statement.execute("ALTER TABLE AUXILIARY.KLADR ADD COLUMN IF NOT EXISTS REGEXP_TYPE_STREET VARCHAR(MAX) DEFAULT ('');");
        statement.execute("ALTER TABLE AUXILIARY.KLADR ADD COLUMN IF NOT EXISTS REGEXP_STREET VARCHAR(MAX) DEFAULT ('');");
